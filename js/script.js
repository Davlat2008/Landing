//alert("крч говоря, ты настолько крутой пока не прочитал этот текст, а сейчас ты стал лохом лохов!")
...
let tabs = document.querySelector('.tarifs_tabs'),
    ...
    tarifDebug = document.querySelectorAll('.during');
...
tabs.addEventListener('click', e => {
    ...
    if (e.target.dataset.time === 'year') {
        ...
        tarifDebug.forEach(during => during.innerHTML = "/year");
    }

    if (e.target.dataset.time === 'month') {
        tarifPrice.forEach(price => price.innerHTML /= 12);
        tarifDebug.forEach(during => during.innerHTML = "/month");
    }
});


// ScrollTop (ракета телепорт)
